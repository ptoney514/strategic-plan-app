...
export function GoalEditForm({ goal, onSave, onCancel }: GoalEditFormProps) {
  ...
  const [showVisualBadge, setShowVisualBadge] = useState(!!goal.indicator_text);
  const [badgeType, setBadgeType] = useState<'on-target' | 'needs-attention' | 'off-target'>('on-target');
  const [customBadgeText, setCustomBadgeText] = useState(goal.indicator_text || 'On Target');
  const [badgeColor, setBadgeColor] = useState(goal.indicator_color || '#22c55e');
  ...
  useEffect(() => {
    if (goal.indicator_text) {
      const text = goal.indicator_text.toLowerCase();
  ...
  return (
    <div className="bg-[#f5f3ef] border-2 border-[#e8e6e1] rounded-xl p-6">
      ...
      {/* Goal Number (Read-only) */}
      <div className="mb-4">
        <label className="block text-[13px] font-semibold text-[#1a1a1a] mb-2">
          Goal Number
        </label>
        <div className="flex items-center gap-2">
          <div className="w-10 h-10 rounded-full bg-[#e8e6e1] flex items-center justify-center">
            <span className="text-[13px] font-semibold text-[#4a4a4a]">
              {goal.goal_number}
            </span>
          </div>
          <span className="text-[12px] text-[#8a8a8a]">
            (Use full goal editor to change goal number)
          </span>
        </div>
      </div>
      ...
      <div className="mb-4">
        ...
        <input
          ...
          placeholder="e.g., Improve Reading Proficiency"
      ...
      <div className="mb-6 pb-4 border-b border-[#e8e6e1]">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <Layers className="h-4 w-4 text-[#10b981]" />
            <span className="text-[13px] font-semibold text-[#1a1a1a]">Status Badge</span>
          </div>
          <button
        ...
        {showVisualBadge && (
          <>
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-2 px-3 py-1.5 border border-[#e8e6e1] rounded-md bg-white">
                <span
